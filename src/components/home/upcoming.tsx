...
export async function Upcoming({ events }: { events: Event[] }) {
    ...
    return (
      <section className="py-40 px-6 md:px-12">
        <div className="max-w-7xl mx-auto text-center">
          ...
          >
            <span>{t.upcoming.emptyReserve}</span>
  ...
  const headlineTitle = pickLocalized(
    headline.title,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (headline as any).title_it,
    locale,
  );
  const headlineTagline = pickLocalized(
    headline.tagline,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (headline as any).tagline_it,
    locale,
  );
        ...
        <div className="flex items-end justify-between mb-20 md:mb-28">
          <BlurIn>
            ...
            <h2 className="font-display font-light leading-[0.95] tracking-[-0.02em] text-[clamp(2.4rem,5.5vw,5rem)]">
              {t.upcoming.headlineA}
              <br />
              <em className="italic text-wine">{t.upcoming.headlineB}</em>
            </h2>
          </BlurIn>
          <Link
            href="/events"
              ...
              <div className="mt-10 flex flex-wrap items-baseline gap-x-10 gap-y-4 text-sm">
                <div>
                  ...
                  <p className="mt-1 font-display text-lg">
                    {formatDateLocalized(headline.starts_at, locale)}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-[0.32em] text-muted">
